...
{
	global $cfg, $sys, $usr;
	if ($sys['parser'] == 'html' && $usr['maingrp'] != 5)
	{
		static $purifier = null;
		// Lazy loading to save performance
		if (is_null($purifier))
		{
			define('HTMLPURIFIER_PREFIX', $cfg['plugins_dir'] . '/htmlpurifier_noadmin/lib/standalone');
			require_once $cfg['plugins_dir'] . '/htmlpurifier_noadmin/lib/HTMLPurifier.standalone.php';
			...
			$config->set('HTML.Doctype', $cfg['plugin']['htmlpurifier_noadmin']['doctype']);
			$config->set('HTML.TidyLevel', $cfg['plugin']['htmlpurifier_noadmin']['tidylevel']);
			$config->set('URI.Base', COT_ABSOLUTE_URL);
			$config->set('URI.Host', $sys['domain']);
			if ($cfg['plugin']['htmlpurifier_noadmin']['rel2abs'])
			{
			...
			if ($usr['id'] > 0)
			{
				...
				if (!file_exists($cfg['plugins_dir'] . "/htmlpurifier_noadmin/presets/htmlpurifier.$preset_name.preset.php"))
				{
					$preset_name = 'default';
				}
			}
			...
			require_once  $cfg['plugins_dir'] . "/htmlpurifier_noadmin/presets/htmlpurifier.$preset_name.preset.php";
			/* config extension */
