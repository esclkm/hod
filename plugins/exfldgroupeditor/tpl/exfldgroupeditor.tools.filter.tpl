...
{PHP.L.Select_cat} : {EXFLD_CAT} <br />
	...
	<input type="hidden" name="l" value="filter" />
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#addoption").click(function () {
			...
			$(object).find('.rtype').attr('name', 'rtype['+qid+']');
			$(object).find('.rvars').attr('name', 'rvars['+qid+']');
			$(object).find('[name^="rfilter"]').attr('name', 'rfilter['+qid+']');
...
<form action="{EXFLD_ACTION}" method="post" name="general">
    <table id="menugenerator" class="cells">
        <tr class="nodrag nodrop">
			...
			<td class="coltop width20">{PHP.L.Type}</td>
			<td class="coltop width20">{PHP.L.ex_vars}</td>
			<td class="coltop width20">{PHP.L.ex_Filter}</td>
