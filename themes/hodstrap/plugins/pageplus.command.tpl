...
<div class="container container-fluid fadeInUpBig wow" data-wow-offset="40" data-wow-duration="1.5s">
	...
	<!-- BEGIN: PAGE_ROW -->
		<div class="col-xs-4 command">
			<div class="command-comment sameh2">
				<div class="command-avatar-img" style="background: url('<!-- IF {PAGE_ROW_MAVATAR} -->{PAGE_ROW_MAVATAR.1|cot_mav_thumb($this, 64, 64, crop)}<!-- ELSE -->{PHP.cfg.mainurl}/themes/{PHP.usr.theme}/images/no-image.png<!-- ENDIF -->') center;"></div>
				<div class="command-author">	
					<strong>{PAGE_ROW_SHORTTITLE|page_color_header}</strong><!-- IF {PAGE_ROW_DESC} -->, {PAGE_ROW_DESC}<!-- ENDIF -->
					<!-- IF {PHP.usr.isadmin} -->
			...
			<div class="command-text sameh"><i class="fa fa-quote-left"></i>{PAGE_ROW_TEXT}</div>
